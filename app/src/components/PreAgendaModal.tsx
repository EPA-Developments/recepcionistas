      ...
      if (agendadas > 0) {
        try {
          await enviarWhatsApp({
            ...
            body: `Segunda Opinión Médica: te dejamos agendadas ${agendadas} ${agendadas === 1 ? 'sesión' : 'sesiones'} de tu ${plan.nombre} este mes. ¡Te esperamos! 💚`,
          });
        } catch {
        ...
        <Stack gap="md">
          ...
          {resultados && (
            <Stack gap="xs">
                ...
                <Stack gap={6}>
                  {resultados.map((r) => (
                    <Group key={r.fecha} gap="xs" wrap="nowrap">
                      {r.ok ? (
                        <IconCheck size={16} color="var(--mantine-color-somAzul-6)" style={{ flexShrink: 0 }} />
                      ) : (
              ...
              {terminado && (
                <Alert color={agendadas === serie.length ? 'somAzul' : 'yellow'} variant="light">
