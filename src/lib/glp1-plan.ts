...
import { CODIGO_CONTROL_GLP1, SERVICIOS_POR_CODIGO } from '../config/catalogo.js';
...
import { diaLocal, estadoTarea, fmtDia, sumarDias, type EstadoRecurso, type Ventana } from './programas.js';

// Piezas comunes de los programas (se reexportan: la app y los bots las importan desde acá).
export {
  claveDe,
  diaLocal,
  estadoTarea,
...
/**
 * ¿El servicio se reserva libre (agenda, ficha)? Los de programa se agendan solo desde
 * su tarea: el control GLP-1 (R-19) y la consulta del Plan Bienestar (R-20), para que
 * cada turno quede atado a su semana o a su día del plan. Sale del catálogo
 * (`soloDesdeTarea`).
 */
export function seAgendaSinTarea(servicioCodigo: string): boolean {
  return !SERVICIOS_POR_CODIGO.get(servicioCodigo)?.soloDesdeTarea;
}
...
export function validarControlSinTarea(servicioCodigo: string): ResultadoValidacion {
  if (servicioCodigo !== CODIGO_CONTROL_GLP1) {
    return { ok: true, bloqueos: [], advertencias: [] };
  }
  return {
    ok: false,
