...
| **Practitioner / PractitionerRole** | Médicos (`src/config/medicos.ts`): identifier `CodeSystem/medico`, matrícula(s) en `identifier` `Identifier/matricula`, `qualification` SNOMED. Su rol: especialidad SNOMED (`specialty`), consultas que atiende (`code` `CodeSystem/servicio`) y si hace el seguimiento del plan (`CodeSystem/rol-profesional`), consultorio (`location`), disponibilidad semanal (`availableTime`) | `split-porcentaje`, `tipo-contrato`; en el rol, `modalidad` (una por modalidad que atiende) |
| **Schedule / Slot** | Agendas: una por consultorio/sala (`recurso-fisico`) y **una por profesional** (R-22; identifier `CodeSystem/medico\|SCH_{codigo}`, `actor` el Practitioner). Las franjas del profesional son de 30 min, identifier `{medico}@{inicio}`, `free` → `busy` al reservar, con la extensión `modalidad` una vez por modalidad en que se puede reservar (la disponibilidad puede ser presencial o teleconsulta por franja); el turno referencia todas sus franjas en `Appointment.slot` | `recurso-fisico`, `profesional`, `modalidad` |
| **Appointment** | Turno reservado (`serviceType` = servicio del catálogo; `specialty` SNOMED; `participant` el Practitioner; consulta del Plan Bienestar: `supportingInformation` → su Task y su CarePlan) | `ocupantes`, `item-tipo`, `item-codigo`, `modalidad` (v3-ActCode `AMB`/`VR`), `teleconsulta-url`, `profesional`, `recurso-fisico`; reserva del portal (R-23): `origen-reserva` (`portal`), `reserva-expira` (hasta cuándo se retiene un `pending` sin seña), `link-pago-sena` |
