...
| **Schedule / Slot** | Agendas: una por consultorio/sala (`recurso-fisico`) y **una por profesional** (R-22; identifier `Identifier/medico\|SCH_{codigo}`, `actor` el Practitioner). Las franjas del profesional son de 30 min, identifier `{medico}@{inicio}`, `free` → `busy` al reservar, con la extensión `modalidad` una vez por modalidad en que se puede reservar (la disponibilidad puede ser presencial o teleconsulta por franja); el turno referencia todas sus franjas en `Appointment.slot` | `recurso-fisico`, `profesional`, `modalidad` |
